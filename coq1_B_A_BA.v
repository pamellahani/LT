...
(*Mq par induction sur 'a' que renverser 2 fois un arbre nous donne le meme arbre  *)
Theorem renva_renva : forall a, renva (renva a) = a.
Proof.
  intro a.
  ...
  (*supposons que ... *)
  induction a as [ (* F *) c  
                 | (* N *) a1 Hrec_a1 a2 Hrec_a2 ]. (* on a 2 hypotheses de recurrence *)

  - cbn [renva]. reflexivity.  (*cas de base: a est un arbre binaire vide *)
  (* trivial : renva a est un arbre binaire vide donc renva (renva a) l'est egalement. *)

  (*cas inductif: a est un noeud avec 2 sous-arbres a1 et a2*)
  - cbn [renva].     
  (*Explication : Les 2 hypotheses de recurrences sont les suivantes : 
      Hrec_a1 : renva (renva a1) = a1
      Hrec_a2 : renva (renva a2) = a2 
    On veut montrer que (renva (renva a1)) (renva (renva a2)) = a1 a2 *)
  rewrite Hrec_a1. (* on applique l'hypothese Hrec_a1 pour simplifier l'expression precedente.*)
  rewrite Hrec_a2. (* idem pour Hrec_a2*)
  reflexivity.     (* on a donc (a1 a2 = a1 a2), donc on a prouve que renva (renva a1) = a *)
Admitted. 
(*Conclusion : on a montre par induction que pour tout arbre binaire a, on a renva (renva a) = a.*)
